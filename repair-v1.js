/* repair-v1.js - SCRIPT DE REPARACIÓN TOTAL (EJECUTAR 1 VEZ) */
...
// 1. LISTA DE DATOS CORRECTA (Verificada con 366 días)
const csvData = `
...
21-05,21 de mayo,🗓️
22-05,22 de mayo,🗓️
23-0Amigo, te pido mil disculpas. Qué frustrante.

Tienes toda la razón. Si viste cómo se borraban y luego en Firebase sigue habiendo solo 12 días, el problema **es mi script de reparación**.

Investigando mi propio código, veo el error. Es un error tonto y te pido perdón por hacerte perder el tiempo.

El problema está en cómo el script procesa la lista de días. La lista `csvData` que te pegué **es correcta**, pero mi bucle `for` para leerla está mal. Está fallando de una manera que solo coge 12 días de cada mes.

---

### Misión 19: El Script de Reparación (V2 - El Bueno)

No vamos a cambiar nombres de archivos. Vamos a usar el `app.js` que ya tienes, pero con el código de reparación **corregido**.

**Por favor, borra los 361 días que hay ahora en tu Firebase** para que la colección `Dias` esté vacía.

#### Paso 1: Reemplazar `app.js`

1.  Ve a tu repositorio y abre el archivo **`app.js`**.
2.  Haz clic en el lápiz (Editar).
3.  **REEMPLAZA** todo el contenido con este **script de reparación CORREGIDO**. He reescrito la lógica de lectura para que sea infalible.

```javascript
/* app.js - SCRIPT DE REPARACIÓN V2 (EL BUENO) */
...
import { getFirestore, doc, setDoc, collection, getDocs, deleteDoc } from "[https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js](https://www.gstatic.com/firebasejs/9.6.1/firebase-firestore.js)";
...
// LA LISTA DE DATOS (ESTO SIEMPRE ESTUVO BIEN)
const csvData = `
ID_Dia,Nombre_Dia,Icono
01-01,1 de enero,🗓️
02-01,2 de enero,🗓️
...
async function repararBaseDeDatos() {
    try {
        ...
        if (!querySnapshot.empty) {
            ...
            borrados = querySnapshot.size;
        }
        contentDiv.innerHTML = `<p>Fase 1 completada: Se eliminaron ${borrados} días corruptos.</p>`;
        
        // --- 2. FASE DE RECARGA (LÓGICA CORREGIDA) ---
        contentDiv.innerHTML += "<p>Fase 2: Cargando 366 días limpios...</p>";
        
        // EL ERROR ESTABA AQUÍ. Ahora usamos 'split' y 'map' de forma segura.
        const lineas = csvData.trim().split(/\r?\n/);
        
        let documentosCargados = 0;

        // Bucle for...of para procesar cada línea
        for (const linea of lineas) {
            
            // Saltar líneas vacías o la cabecera
            if (linea.trim() === '' || linea.startsWith('ID_Dia')) {
                continue;
            }
            ...
            if (valores.length >= 3) {
                ...
                if (ID_Dia) { // Solo si el ID_Dia no está vacío
                    const diaData = {
        ...
        contentDiv.innerHTML = `
            ...
            <p>Vuelve a GitHub y reemplaza este script por el código final de la app (Misión 20).</p>
        `;
