...
async function abrirModalPreview(dia) {
    console.log("Opening preview:", dia.id); currentlyOpenDay = dia; let modal = document.getElementById('preview-modal'); if (!modal) { modal = document.createElement('div'); modal.id = 'preview-modal'; modal.className = 'modal-preview'; modal.innerHTML = ` <div class="modal-preview-content"> <div class="modal-preview-header"> <h3 id="preview-title"></h3> <button id="edit-from-preview-btn" title="Edit this day">${pencilIconSVG}</button> </div> <div class="modal-preview-memorias"> <h4>Memories:</h4> <div id="preview-memorias-list">Loading...</div> </div> <button id="close-preview-btn" class="aqua-button">Close</button> </div>`; document.body.appendChild(modal); document.getElementById('close-preview-btn').onclick = () => cerrarModalPreview(); modal.onclick = (e) => { if (e.target.id === 'preview-modal') cerrarModalPreview(); }; // Corrected listener assignment below
         // Use addEventListener for the edit button to ensure it works after modal creation
         const editBtn = document.getElementById('edit-from-preview-btn');
         if (editBtn) {
             // Remove potential old listener before adding new one
             editBtn.replaceWith(editBtn.cloneNode(true)); // Simple way to remove listeners
             document.getElementById('edit-from-preview-btn').addEventListener('click', () => {
                 if (currentlyOpenDay) {
                     cerrarModalPreview();
                     // Use a slightly longer delay to ensure preview is fully hidden
                     setTimeout(() => abrirModalEdicion(currentlyOpenDay), 250);
                 } else { console.error("No day data for edit button"); }
             });
         }
...
async function abrirModalEdicion(dia) { // dia can be null if adding via footer button
    ...
    if (isAdding) {
     ...
     if (!currentlyOpenDay || !currentlyOpenDay.id) { console.error("Invalid day data."); alert("Error loading day data."); return; }
            ...
            <div class="modal-content">
                <div class="modal-content-scrollable"> {/* Scroll container */}
                    ...
                    <div class="modal-section memorias-section">
                        ...
                        <form id="memory-form">
                             <p class="section-description" id="memory-form-title">Add New Memory</p>
...
async function cargarYMostrarMemorias(diaId, targetDivId) {
    ...
    try {
        ...
        memoriasListDiv.appendChild(fragment); if (targetDivId === 'edit-memorias-list') attachMemoryActionListeners(diaId); console.log(`Loaded ${querySnapshot.size} memories for ${diaId} into ${targetDivId}`);
