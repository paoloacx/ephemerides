/* app.js - SCRIPT DE CARGA FINAL (EJECUTAR 1 VEZ) */
...
// ¡AQUÍ ESTÁ LA MAGIA! Todos tus datos del CSV pegados directamente
const csvData = `
ID_Dia (Texto),Nombre_Dia (Texto),Icono (Texto)
01-01,1 de enero,🗓️
02-01,2 de enero,🗓️
03-01,3 de enero,🗓️
04-01,4 de enero,🗓️
...
async function cargarDias() {
    ...
    try {
        ...
        // 1. Procesar las líneas desde la variable csvData
        const lineas = csvData.trim().split(/\r?\n/).filter(line => line.trim() !== '');

        if (lineas.length <= 1) {
             contentDiv.innerHTML = "<p>Error de lectura. El archivo está vacío.</p>";
             return;
        }

        let documentosCargados = 0;
        
        // 2. Iterar sobre los días (empezamos en la línea 1 para saltar los encabezados)
        for (let i = 1; i < lineas.length; i++) {
            ...
            if (!ID_Dia) { // Saltar si el ID está vacío
                continue; 
            }
            ...
            // 3. Guardar el documento en la colección 'Dias'
            // NOTA: Esto SOBRESCRIBIRÁ tu '01-01' de prueba, lo cual está bien.
        ...
        contentDiv.innerHTML = `
            ...
            <hr>
            <h3>✅ ¡La base de datos está llena!</h3>
            <p><strong>Siguiente paso:</strong> Vuelve a GitHub y reemplaza este script por el código final de la app (Misión 14).</p>
        `;

    } catch (error) {
        console.error("Error en la carga masiva:", error);
        contentDiv.innerHTML = `<p>Error al cargar. ${error.message}</p>`;
    }
}
