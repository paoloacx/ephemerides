/* app.js - v9.0 - Login, Advanced Add Memory UI, Visual Polish */

// Importaciones (Añadir Auth y Storage)
...
onAuthStateChanged(auth, (user) => {
    ...
    if (user) {
        ...
        // Maybe reload data or enable features based on user?
        // For now, just update UI
    } else {
        console.log("User logged out.");
        // Handle logout if needed (e.g., clear user-specific data)
    }
});
...
function updateLoginUI(user) {
    ...
    if (user) {
        // Show user info and logout icon
        if (userInfo) userInfo.style.display = 'flex';
        if (userName) userName.textContent = user.displayName || user.email || 'User';
        if (userImg) userImg.src = user.photoURL || 'https://placehold.co/40x40/ccc/fff?text=?'; // Placeholder avatar
        if (loginBtn) {
        ...
        // Show login icon/text
        if (userInfo) userInfo.style.display = 'none';
        if (loginBtn) {
            // Show Google Icon or a simple Login icon/text
             // Simple Google logo (replace with a better SVG or image if desired)
            loginBtn.innerHTML = `<img src="https://www.google.com/images/branding/googlelogo/1x/googlelogo_color_272x92dp.png" alt="G" style="width: 20px; height: auto;">`;
...
async function checkAndRunApp() {
    ...
    try {
        ...
        // Initial setup for login button listener (will be updated by onAuthStateChanged)
        const loginBtn = document.getElementById('login-btn');
        if(loginBtn) loginBtn.onclick = handleLogin;
        updateLoginUI(auth.currentUser); // Update UI based on initial state

    } catch (e) { appContent.innerHTML = `<p class="error">Critical error during startup: ${e.message}</p>`; console.error(e); }
}

// --- generateCleanDatabase (No changes) ---
async function generateCleanDatabase() {
    ...
    try {
        ...
        if (!oldDocsSnapshot.empty) {
            ...
            for (const docSnap of oldDocsSnapshot.docs) { batch.delete(docSnap.ref); deleteCount++; if (deleteCount >= 400) { console.log(`Committing delete batch (${deleteCount})...`); await batch.commit(); batch = writeBatch(db); deleteCount = 0; } }
    ...
    try {
        for (let m = 0; m < 12; m++) {
            const monthNum = m + 1, monthStr = monthNum.toString().padStart(2, '0'); const numDays = daysInMonth[m];
            for (let d = 1; d <= numDays; d++) {
                const dayStr = d.toString().padStart(2, '0'); const diaId = `${monthStr}-${dayStr}`; const diaData = { Nombre_Dia: `${d} ${monthNames[m]}`, Icono: '', Nombre_Especial: "Unnamed Day" };
        ...
        if (ops > 0) { console.log(`Committing final generate batch (${ops})...`); await batch.commit(); }
         console.log(`--- Regeneration complete: ${created} days created ---`); appContent.innerHTML = `<p class="success">✅ Database regenerated with ${created} days!</p>`;
    } catch(e) { console.error("Error generating days:", e); throw e; }
}

// --- loadDataAndDrawCalendar (No changes) ---
...
// --- configurarFooter (No changes) ---
function configurarFooter() {
    document.getElementById('btn-hoy').onclick = () => {
        const today = new Date(); const todayMonth = today.getMonth(); const todayDay = today.getDate(); const todayId = `${(todayMonth + 1).toString().padStart(2, '0')}-${todayDay.toString().padStart(2, '0')}`;
        const todayDia = allDaysData.find(d => d.id === todayId);
        if (todayDia) { if (currentMonthIndex !== todayMonth) { currentMonthIndex = todayMonth; dibujarMesActual(); setTimeout(() => abrirModalPreview(todayDia), 50); } else { abrirModalPreview(todayDia); } window.scrollTo(0, 0); }
            ...
            results.forEach(mem => {
                const itemDiv = document.createElement('div'); itemDiv.className = 'memoria-item search-result'; let fechaStr = 'Unknown date'; if (mem.Fecha_Original?.toDate) { try { fechaStr = mem.Fecha_Original.toDate().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }); } catch(e) { fechaStr = mem.Fecha_Original.toDate().toISOString().split('T')[0]; } } else if (mem.Fecha_Original) { fechaStr = mem.Fecha_Original.toString(); }
                 ...
                 switch (mem.Tipo) {
                     ...
                     case 'Musica': if (mem.CancionData?.trackName) contentHTML += `🎵 Listened to: <strong>${mem.CancionData.trackName}</strong> by ${mem.CancionData.artistName}`; else contentHTML += `🎵 Listened to: ${mem.CancionInfo || 'Unknown Song'}`; break;
                     case 'Imagen': contentHTML += `🖼️ Added Image`; if (mem.ImagenURL) contentHTML += ` (<a href="${mem.ImagenURL}" target="_blank" style="font-size: 10px;">View</a>)`; if (mem.Descripcion) contentHTML += `<br>${mem.Descripcion}`; break;
...
function cerrarModalPreview() { /* ... unchanged ... */ const modal = document.getElementById('preview-modal'); if (modal) { modal.classList.remove('visible'); setTimeout(() => { modal.style.display = 'none'; }, 200); } currentlyOpenDay = null; }

// --- Edit Modal (Fix Edit Button Listener Attachment) ---
async function abrirModalEdicion(dia) {
    ...
    resetMemoryForm(); document.getElementById('edit-modal-title').textContent = `Editing: ${dia.Nombre_Dia} (${dia.id})`;
    const inputNombreEspecial = document.getElementById('nombre-especial-input'); inputNombreEspecial.value = dia.Nombre_Especial === 'Unnamed Day' ? '' : dia.Nombre_Especial;
    ...
    const addMemoriaForm = document.getElementById('add-memoria-form'); addMemoriaForm.onsubmit = null; addMemoriaForm.onsubmit = async (e) => { e.preventDefault(); const fechaInput = document.getElementById('memoria-fecha').value; const descInput = document.getElementById('memoria-desc').value; if (editingMemoryId) { await updateMemoria(dia.id, editingMemoryId, fechaInput, descInput.trim()); } else { await guardarNuevaMemoria(dia.id, fechaInput, descInput.trim()); } };
    modal.style.display = 'flex'; setTimeout(() => modal.classList.add('visible'), 10);
    // Load memories AFTER modal is potentially created and visible
    await cargarYMostrarMemorias(dia.id, 'edit-memorias-list');
}
function cerrarModalEdicion() { /* ... unchanged ... */ const modal = document.getElementById('edit-modal'); if (modal) { modal.classList.remove('visible'); setTimeout(() => { modal.style.display = 'none'; resetMemoryForm(); }, 200); } currentlyOpenDay = null; }

// --- Load/Display Memories (Fix Edit/Delete Listener Attachment) ---
async function cargarYMostrarMemorias(diaId, targetDivId) {
    ...
    try {
        const memoriasRef = collection(db, "Dias", diaId, "Memorias"); const q = query(memoriasRef, orderBy("Fecha_Original", "desc"));
        const querySnapshot = await getDocs(q); if (querySnapshot.empty) { memoriasListDiv.innerHTML = '<p style="font-style: italic; color: #777; font-size: 12px;">No memories yet.</p>'; return; }
        memoriasListDiv.innerHTML = '';
        querySnapshot.forEach((docSnap) => {
            const memoria = { id: docSnap.id, ...docSnap.data() }; currentMemories.push(memoria);
            const itemDiv = document.createElement('div'); itemDiv.className = 'memoria-item'; let fechaStr = 'Unknown date'; if (memoria.Fecha_Original?.toDate) { try { fechaStr = memoria.Fecha_Original.toDate().toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' }); } catch(e) { fechaStr = memoria.Fecha_Original.toDate().toISOString().split('T')[0]; } } else if (memoria.Fecha_Original) { fechaStr = memoria.Fecha_Original.toString(); }
            ...
            switch (memoria.Tipo) {
                ...
                case 'Musica': if (memoria.CancionData?.trackName) { contentHTML += `🎵 Listened to: <strong>${memoria.CancionData.trackName}</strong> by ${memoria.CancionData.artistName}`; if(memoria.CancionData.artworkUrl60) artworkHTML = `<img src="${memoria.CancionData.artworkUrl60}" alt="Artwork" class="memoria-artwork">`; } else { contentHTML += `🎵 Listened to: ${memoria.CancionInfo || 'Unknown Song'}`; } break;
                case 'Imagen': contentHTML += `🖼️ Added Image`; if (memoria.ImagenURL) contentHTML += ` (<a href="${memoria.ImagenURL}" target="_blank" style="font-size: 10px;">View</a>)`; if (memoria.Descripcion) contentHTML += `<br>${memoria.Descripcion}`; break;
                case 'Texto': default: contentHTML += memoria.Descripcion || 'No description'; break;
            }
            const actionsHTML = (targetDivId === 'edit-memorias-list') ? ` <div class="memoria-actions"> <button class="edit-btn" title="Edit">${editIconSVG}</button> <button class="delete-btn" title="Delete">${deleteIconSVG}</button> </div>` : '';
            ...
            if (targetDivId === 'edit-memorias-list') {
                ...
                if (editBtn) editBtn.onclick = () => startEditMemoria(memoria);
                if (deleteBtn) {
                    const displayInfo = memoria.Descripcion || memoria.LugarNombre || memoria.CancionInfo || memoria.ImagenURL || "this memory";
                    deleteBtn.onclick = () => confirmDeleteMemoria(diaId, memoria.id, displayInfo);
                }
            }
...
async function saveMemoryFromAddModal() {
    ...
    switch (type) {
        ...
        case 'Imagen': const fileInput = document.getElementById('add-mem-image-upload'); memoryData.Descripcion = document.getElementById('add-mem-image-desc').value.trim() || null; if (fileInput.files && fileInput.files[0]) { imageFileToUpload = fileInput.files[0]; memoryData.ImagenURL = "placeholder_uploading"; isValid = true; document.getElementById('image-upload-status').textContent = `Ready: ${imageFileToUpload.name}`; } else { isValid = false; } break; default: isValid = false; break;
    }
    ...
    try {
        if (type === 'Imagen' && imageFileToUpload) {
            statusDiv.textContent = 'Image upload pending implementation...';
            alert("Image upload functionality is not yet implemented. Memory not saved."); // User feedback
            statusDiv.textContent = 'Save cancelled: Image upload pending.'; statusDiv.className = 'error'; return; // Stop saving
            // ** Future implementation: **
            // statusDiv.textContent = 'Uploading image...';
            // const imageUrl = await uploadImage(diaId, imageFileToUpload); // Call upload function
            // memoryData.ImagenURL = imageUrl;
            // await saveMemoryData(diaId, memoryData); // Then save
        } else {
...
function resetAddMemoryFormFields() {
    ...
    selectedPlace = null;
    // Don't reset day/year/type
}

/* // ** Placeholder for Image Upload Function **
async function uploadImage(diaId, file) {
    // Requires Firebase Storage to be initialized: const storage = getStorage(app);
    // Requires Storage library import: import { getStorage, ref, uploadBytes, getDownloadURL } from "...";
    if (!storage) { throw new Error("Firebase Storage not initialized."); }
    console.log(`Uploading image for ${diaId}: ${file.name}`);
    // Create a storage reference (e.g., /images/MM-DD/timestamp-filename.jpg)
    const filePath = `images/${diaId}/${Date.now()}-${file.name}`;
    ...
    try {
        ...
        throw error; // Rethrow to handle in saveMemoryFromAddModal
    }
} */
...
window.handleLogin = handleLogin; // Expose login
window.handleLogout = handleLogout; // Expose logout
